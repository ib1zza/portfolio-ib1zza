...
const OneProjectCard = ({ projectInfo }: Props) => {
    ...
    return (
        <motion.div
            ...
            className={s.projects__slider__item}
            key={projectInfo.id}
        >
            <div className={s.projects__image}>
                <Link
                    to={isCloned ? "#" : AppRoutes.PROJECTS + "/" + projectInfo.id}
                    tabIndex={isCloned ? -1 : 0}
                    aria-hidden={isCloned}
                    aria-label={"Go to project"}
                    onClick={(e) => {
                        if (isCloned) e.preventDefault();
                    }}
                >
                    <motion.div className={s.project__link}>
                        <motion.img
                            variants={{
                                initial: { scale: 1 },
                                hov: { scale: 1.1, transition: { duration: 0.3 } },
                            }}
            ...
            <div className={s.project__item__text}>
                ...
                <div className={s.project__info}>
                    ...
                    <Button
                        ...
                        onClick={() => {
                            if (!isCloned) {
                                navigate(AppRoutes.PROJECTS + "/" + projectInfo.id);
                            }
                        }}
                    >
                        {t("view")}
